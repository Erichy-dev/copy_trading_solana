...
async function retriveWalletState(wallet_address: string) {
  try {
    ...
    const MIN_BALANCE = 0.000001; // Minimum balance threshold
    
    // Always get and set SOL balance, even if 0
    const solBalance = await connection.getBalance(
      new PublicKey(wallet_address)
    );
    results["SOL"] = solBalance / 10 ** 9; // Always include SOL balance
    ...
    accounts.forEach((account) => {
      ...
      // Only add tokens with balance above minimum threshold
      if (tokenBalance >= MIN_BALANCE) {
    ...
    console.error("Error in retriveWalletState:", e);
    return { "SOL": 0 }; // Return default state with 0 SOL on error
  }
}
